...
/// <summary>
/// Recovery path for when the user can't log in with their pattern (new PC, forgot the pattern,
/// etc). Answers are never stored - like the pattern, they only ever exist transiently to derive
/// a key that unwraps the same DEK used everywhere else, so recovering via security questions
/// and logging in via pattern reach the exact same vault.
/// </summary>
