...
/// <summary>
/// Draws and confirms a pattern for the hidden vault. If one already exists, first requires the
/// current hidden pattern (Settings runs inside the main vault's session, so the hidden vault's
/// DEK isn't otherwise available to re-wrap under a new pattern).
/// </summary>
